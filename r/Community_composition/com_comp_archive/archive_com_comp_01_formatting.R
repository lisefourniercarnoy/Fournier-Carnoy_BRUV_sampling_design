...
write.csv(spabal_metadata, file = paste0("data/tidy/2024_geographe_cw_tidy_metadata.csv"))
...
cw_tidy_maxn <- maxn %>%
  ...
  mutate(sd = ifelse(grepl('P', opcode), "preferential", "spatially balanced")) %>%
  glimpse()
saveRDS(cw_tidy_maxn, file = paste0("data/tidy/2024_geographe_cw_tidy_maxn.rds"))
...
write.csv(pref_metadata, file = paste0("data/tidy/2024_geographe_state_tidy_metadata.csv"))
...
state_tidy_maxn <- maxn %>%
  ...
  dplyr::select(opcode, fullspp, MaxN, longitude, latitude, depth, status) %>%
  dplyr::filter(grepl('EGB', opcode, fixed = T)) %>% # Selecting East Geographe drops only
  mutate(sd = "preferential") %>%
  glimpse()
saveRDS(state_tidy_maxn, file = paste0("data/tidy/2024_geographe_state_tidy_maxn.rds"))
...
dat <- dat[, c("sd", setdiff(names(dat), "sd"))]
