...
# Note : the entirety of the script (for 1 site) should yield 15 files 
...
library(ggnewscale) # for plot colours
library(ggspatial) # for scalebars on plots
library(nngeo) # for nearest neighbour distances
#library(cubelyr)
...
file_bathy_deriv        <- paste0("data/tidy/D01_", study_site, "_bathy_predictors.rds")
...
min_depth <- ifelse(study_site == "waatern", 7, 20)
depth_mask <- preds$bathy_250m < -min_depth # Select the depth to cut out
plot(depth_mask)
...
saveRDS(samp_area, paste0("QGIS layers/produced_from_code/D04_", study_site, "_sampling_area_deeper_than_", min_depth, "m.rds"))

## Make strata ----------------------------------------------------------------

# Using detrended bathymetry - same as 2024 SB design
hist(preds$detrended_250m)
detrended_qs <- if(study_site == "waatern") {detrended_qs <- c(0, 0.5, 0.9, 1)} else {detrended_qs <- c(0, 0.5, 0.9, 1)}
...
saveRDS(sf_all, paste0("outputs/distribution_modelling_outputs/D04_", study_site, "_sample_area_detrended_strata.rds"))
...
saveRDS(inp_sf, paste0("outputs/distribution_modelling_outputs/D04_sampling_designs/D04_", study_site, "_sampling_design_strata_sample_numbers.rds")) # Save to use in other scripts
...
base_samps <- tibble("all" = 36) # for GRTS to know how many samples to put
...
saveRDS(samples_cleaned, paste0('outputs/distribution_modelling_outputs/D04_sampling_designs/D04_', study_site, '_example_sampling_design_simple_spatial_balance.rds')) # for use in Rmarkdown
...
saveRDS(sampling_designs, file = paste0("outputs/distribution_modelling_outputs/D04_sampling_designs/D04_", study_site, "_simple_spatially_balanced_designs.rds"))
...
if (study_site == "waatern") {
  base_samps <- tibble(
    "SZ_strata_1" = 7,
    "SZ_strata_2" = 8,
    "SZ_strata_3" = 10, # highest strata
    "out_strata_1" = 7,
    "out_strata_2" = 8,
    "out_strata_3" = 10 # highest strata
  )
} else {
  base_samps <- tibble(
    "SZ_strata_1" = 5,
    "SZ_strata_2" = 6,
    "SZ_strata_3" = 7, # highest strata
    "out_strata_1" = 5,
    "out_strata_2" = 6,
    "out_strata_3" = 7 # highest strata
  )
}
...
saveRDS(samples_cleaned, paste0('outputs/distribution_modelling_outputs/D04_sampling_designs/D04_', study_site, '_example_sampling_design_stratified_spatial_balance_25_25.rds')) # for use in Rmarkdown
...
# Save the list of sampling designs
saveRDS(sampling_designs, file = paste0("outputs/distribution_modelling_outputs/D04_sampling_designs/D04_", study_site, "_stratified_spatial_balanced_designs_25_25.rds"))
...
if (study_site == "waatern") {
  base_samps <- tibble(
    "SZ_strata_1" = 5,
    "SZ_strata_2" = 7,
    "SZ_strata_3" = 8, # highest strata
    "out_strata_1" = 8,
    "out_strata_2" = 10,
    "out_strata_3" = 12 # highest strata
  )
} else {
  base_samps <- tibble(
    "SZ_strata_1" = 3,
    "SZ_strata_2" = 5,
    "SZ_strata_3" = 6, # highest strata
    "out_strata_1" = 9,
    "out_strata_2" = 7,
    "out_strata_3" = 6 # highest strata
  )
}
...
saveRDS(samples, paste0('outputs/distribution_modelling_outputs/D04_sampling_designs/D04_', study_site, '_example_sampling_design_stratified_spatial_balance_20_30.rds')) # for use in Rmarkdown
...
# Save the list of sampling designs
saveRDS(sampling_designs, file = paste0("outputs/distribution_modelling_outputs/D04_sampling_designs/D04_", study_site, "_stratified_spatial_balanced_designs_20_30.rds"))
...
saveRDS(samples_cleaned, paste0('outputs/distribution_modelling_outputs/D04_sampling_designs/D04_', study_site, '_example_sampling_design_preferential_25_25.rds')) # for use in Rmarkdown
...
# Save the list
saveRDS(sampling_designs, file = paste0("outputs/distribution_modelling_outputs/D04_sampling_designs/D04_", study_site, "_preferential_designs_25_25.rds"))
...
saveRDS(samples, paste0('outputs/distribution_modelling_outputs/D04_sampling_designs/D04_', study_site,'_example_sampling_design_preferential_20_30.rds')) # for use in Rmarkdown
...
# Save the list
saveRDS(sampling_designs, file = paste0("outputs/distribution_modelling_outputs/D04_sampling_designs/D04_", study_site, "_preferential_designs_20_30.rds"))
...
if(study_site == "waatern") {lat_split <- -33.51} else {lat_split <- -34.09} # set the latitude split between the north and the south of SZ, this is obviously different depending on the study_site
sf_north  <- sf_all_clump %>% filter(in_SZ == FALSE & latitude > lat_split) %>% dplyr::select(-centroid, -latitude) %>% st_union() %>% st_sf(in_SZ = "FALSE_north", geometry = .); plot(sf_north$geometry)
sf_south  <- sf_all_clump %>% filter(in_SZ == FALSE & latitude <= lat_split) %>% dplyr::select(-centroid, -latitude) %>% st_union() %>% st_sf(in_SZ = "FALSE_south", geometry = .); plot(sf_south$geometry)
...
pts_per_cluster_out  <- ifelse(study_site == "waatern", 8, 6)  # Number of points per cluster outside SZ
pts_per_cluster_SZ   <- ifelse(study_site == "waatern", 9, 6)  # Number of points per cluster inside SZ (this is to add up to 50 total points)
...
generate_clumped_points <- function(cluster_centers_utm, samp_area_utm,
                                    ...
                                    SZ,
                                    min_distance,
                                    points_per_cluster_SZ = pts_per_cluster_SZ,
                                    points_per_cluster_out = pts_per_cluster_out) {
  clumped_points <- list()
...
saveRDS(samples_cleaned, paste0('outputs/distribution_modelling_outputs/D04_sampling_designs/D04_', study_site, '_example_sampling_design_clustered.rds')) # for use in Rmarkdown
...
#below are lines that are needed for rerunning the loop below. for each design that fails (because takes >60sec to generate) we'll skit it and rerun it.
#null_indices <- which(sapply(sampling_designs, is.null))
new_designs <- vector("list", length = 1000-length(sampling_designs))
null_indices <- which(sapply(new_designs, is.null))

for (i in null_indices) {      
#for (i in 1:(1000-length(sampling_designs))) { #num_designs

  cat("Iteration:", i, "\n")
  
  result <- tryCatch({
    withTimeout(timeout = 60, onTimeout = "error", {
      ...
      cluster_centers_sf <- st_transform(cluster_centers, crs)
      ...
      # Store the result
      cat("Returning result with", nrow(samples_cleaned), "points\n")
      ...
      plot(cluster_centers_wgs$geometry, col = "#A3320B", pch = 4, cex = 1.2, main = paste("Simulation number", i))
      plot(samples$geometry, col = "#EFA00B", pch = 20, add = TRUE)
      plot(SZ_wgs$geometry, border = "#3E6990", lwd = 2, add = TRUE)
    })
  }, TimeoutException = function(ex) {
    cat("Timeout on iteration", i, "- Skipping\n\n")
    NULL
  }, error = function(e) {
  ...
  # If timeout or error occurred, skip storing anything
  if (!is.null(result)) {
    cat("Storing result for iteration", i, "\n")
    #sampling_designs[[i]] <- result
    new_designs[[i]] <- result
    
    } else {
    cat("Result is NULL for iteration", i, "\n")
    #sampling_designs[[i]] <- NULL
    new_designs[[i]] <- result
    }
}
length(sampling_designs)
length(new_designs)

d <- c(sampling_designs, new_designs)
length(d)


saveRDS(d, file = paste0("outputs/distribution_modelling_outputs/D04_sampling_designs/D04_", study_site, "_clustered_designs.rds"))


sampling_designs <- readRDS(paste0("outputs/distribution_modelling_outputs/D04_sampling_designs/D04_", study_site, "_clustered_designs.rds"))
