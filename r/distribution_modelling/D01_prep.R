...
library(sf) # for manipulating spatial features
library(terra) # for manipulating spatial features
...
file_metadata <- ifelse(study_site == "waatern",
                        "data/raw/2024_waatern_commonwealth/2024-04_Geographe_stereo-BRUVs_Metadata.rds", # raw export from labsheets
                        "data/raw/2024_waatu_commonwealth/2024-10_SwC_stereo-BRUVs_Metadata.csv")

file_maxn <- ifelse(study_site == "waatern", 
                    "data/tidy/00_2024_waatern_BRUVs_tidy_counts.rds", # made/checked in script 00_checkEM
                    "data/tidy/00_2024_waatu_BRUVs_tidy_counts.rds")
...
metadata <- if (study_site == "waatern") {
  metadata <- readRDS(file_metadata) %>% rename(opcode = sample)
} else {
...
dat <- readRDS(file_maxn) %>% 
  rename(fullspp = scientific,
         opcode = sample,
         maxn = count) %>% 
  ...
  mutate(
    longitude = coalesce(as.numeric(longitude_dd.x), as.numeric(longitude_dd.y)),
    latitude = coalesce(as.numeric(latitude_dd.x), as.numeric(latitude_dd.y)),
    #depth   = coalesce(as.numeric(depth.x), as.numeric(depth.y)),  # convert to numeric if needed
    status  = coalesce(status.x, status.y),
  ) %>%
  dplyr::select(opcode, fullspp, maxn, depth_m, longitude, latitude, status) %>% 
...
saveRDS(dat, paste0("data/tidy/D01_", study_site, "_presence_latlong.rds")) # Needed for 03_gam
...
if (study_site == "waatu") {
  ...
  tidy.habitat <- merge(test_hab, metadata, by = "opcode") %>% 
    glimpse()
} # this statement cleans up the habitat data in a format we can use
...
if (study_site == "waatern") {
  ...
  # this is from  Transect-Measure exports. if the data is exported through Squidle (like Waatu is), look at the Waatu workflow
  ...
  tidy.habitat <- habitat %>%
    filter(!level_2 %in% c("", "Unscorable", NA)) %>%  # remove unscorable
    mutate(number = 1) %>%
    group_by(campaignid, sample, level_2, level_3, level_4, level_5, scientific) %>% # group and tally up
    summarise(number = n(), .groups = "drop") %>%
    left_join(
      metadata %>% dplyr::select(sample, longitude_dd, latitude_dd),
      by = "sample"
    ) %>%
    rename(opcode = sample) %>%
    dplyr::select(campaignid, opcode, level_2, level_3, level_4, level_5,
                  scientific, number, longitude_dd, latitude_dd)
} # this statement reads waatern habitat data and checks it.


# further clean up, and save
if (study_site == "waatern") {
  habi <- tidy.habitat %>% # Observed habitat
  ...
  habi <- tidy.habitat %>% 
    ...
    mutate(number = 1) %>% # this is to sum up rows
    
    # filter out unscorables
    filter(!(habitat %in% c("Unscorable", "Fishes"))) %>% 
    
    # make the correct categories
    mutate(taxa = case_when(habitat %in% c("Encrusting", "Erect fine branching", "Large canopy-forming",
                                           "Macroalgae", "Erect coarse branching", "Ecklonia radiata", 
                                           "Scytothalia dorycarpa", "Filamentous / filiform") ~ "macro",
                            habitat %in% "Unconsolidated (soft)" ~ "sand",
                            habitat %in% "Consolidated (bare rock)" ~ "rock",
                            habitat %in% "Thalassodendron spp" ~ "seagrass",
                            habitat %in% c("Cup-likes", "Hydroids", "Massive forms", "Sponges", "Crusts", "Hydrocorals",
    ...
    st_as_sf(coords = c("longitude_dd", "latitude_dd"), crs = 4326) %>%  # Replace with the correct column names
    glimpse()  
} # this statement formats the habitat data correctly depending on the original dataset's format 

saveRDS(habi, file = here::here(paste0("data/tidy/D01_", study_site, "_tidy_habitat.rds")))
...
saveRDS(predictors, file = paste0("data/tidy/D01_", study_site, "_bathy_predictors.rds"))
