...
mature_pred_PS      <- as.data.frame(readRDS("outputs/Length_comparison/predicted_abundance_rasters/SZ_increase/mature_predicted_abundance_Chrysophrys_auratus_full_data_SZ_increase.rds"), xy=TRUE)

mature_pred_SMW     <- as.data.frame(readRDS("outputs/Length_comparison/predicted_abundance_rasters/SZ_increase/mature_predicted_abundance_Ophthalmolepis_lineolatus_full_data_SZ_increase.rds"), xy=TRUE)

```

```{r example_sampling_design, fig.cap = 'Examples of a. Spatially balanced sampling design simulation, b. Preferential sampling design simulation and c. clustered sampling design simulation around the simulated NTZ.  D. Predicted abundance around the simulation area with 80% increase in C. auratus within the simulated NTZ.', echo = FALSE, message = FALSE, warning = FALSE}
...
species_list <- c("Chrysophrys auratus", "Glaucsoma hebraicum", "Ophthalmolepis lineolatus")  # Add all 3 species here

# Environmental variables list
pred.vars <- c("bathy_250m", "roughness_250m", "detrended_250m", 
               "reef_250m", "sand_250m", "seagrass_250m")

# Function to clean data, fit GAM model and tidy results
process_species <- function(model_spp, species_list, pred.vars) {
  
  # Filter data for the given species
  dat_filtered <- dat[dat$full_spp %in% model_spp,] %>%
    group_by(latitude, longitude, ID, !!!syms(pred.vars)) %>%
    summarise(
  ...
  # Load best model for the species
  best_model_name <- readRDS(paste0("data/rmd/gam_best_model_", gsub(" ", "_", model_spp), ".rds"))
  
  # Extract variables from the best model
  best_model_vars <- strsplit(best_model_name, "\\+")[[1]]
  
  # Format the variables for the GAM model
  ...
  # Define a function to fit the GAM model
  make_optimal_model <- function(vars, family) {
    formula_str <- paste("count_mature ~", vars)
    model <- gam(as.formula(formula_str), data = dat_filtered, method = "REML", family = family)
    return(model)
  }
  
  # Fit the model
  m_mature <- make_optimal_model(best_vars, tw())
  ...
  # tidy_model$Variable <- gsub("s\\(seagrass_250m\\)", "Seagrass (250m resolution)", tidy_model$Variable)
  # tidy_model$Variable <- gsub("s\\(bathy_250m\\)", "Bathymetry (250m resolution)", tidy_model$Variable)
...
# Loop over the species and process each one
all_species_results <- lapply(species_list, function(species) process_species(species, pred.vars = pred.vars))

# The result is a list of tidy model outputs for each species
names(all_species_results) <- species_list
colnames(dat)
...
best_model_name_PS <- readRDS("data/rmd/gam_best_model_Glaucosoma_hebraicum.rds") # variables of best model
...
From all the BRUV samples collected in ____, a distribution model was fitted to bathymetry and habitat covariates. The optimal model included sand, seagrass and detrended bathymetry as covariates, all from the 250m resolution dataset. The final model explained `r round(((1 - (deviance(m_mature_PS) / deviance(update(m_mature_PS, . ~ 1))))*100), 2)`% of deviance in the data.
