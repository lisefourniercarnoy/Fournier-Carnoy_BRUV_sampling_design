...
plot <- ggplot() +
  ...
  geom_sf(data = com_samp_area,
          aes(fill = 'Community \ncomposition \nanalysis limits'), color = "#394F49", size = 5) +
  geom_sf(data = AMP[AMP$ZoneName == 'National Park Zone',],
          aes(fill = 'No-take \nzones'), alpha = 0.5, color = "#65743A", size = 0.5) +
  geom_sf(data = MPA,
          aes(fill = 'No-take \nzones'), alpha = 0.5, color = "#65743A", size = 0.2) +
...
final_table %>%
  ...
  group_rows("b. Simulated BRUVs (clustered sampling design)", 4, 4) %>% 
  group_rows("c. Simulated BRUVs (preferential sampling design)", 5, 5) %>% 
  group_rows("d. Simulated BRUVs (spatially balanced sampling design)", 6, 8)
...
To compare the sampling designs’ ability to detect the effect of a NTZ on single species, we conducted simulations of sampling designs on a modelled abundance distribution of Yijarup/Pink Snapper (*C. auratus*) and (Wadandi name?)/Southern Maori Wrasse around a simulated NTZ.

After modelling the distribution predictions of multiple species present in the dataset, these two species were chosen as they broadly represent two different spatial distribution strategies: Yijarup/Pink Snapper is highly mobile and predictions indicated a weak association with bathymetry and habitat feature, while (Wadandi name?)/Southern Maori Wrasse are less mobile and more associated to reef features. The performance of different sampling designs could therefore be compared for different species distribution types.
...
mature_pred_PS      <- as.data.frame(readRDS("outputs/Length_comparison/predicted_abundance_rasters/SZ_increase/mature_predicted_abundance_Chrysophrys_auratus_SZ_increase.rds"), xy=TRUE)

mature_pred_SMW     <- as.data.frame(readRDS("outputs/Length_comparison/predicted_abundance_rasters/SZ_increase/mature_predicted_abundance_Ophthalmolepis_lineolatus_SZ_increase.rds"), xy=TRUE)

```

```{r example_sampling_design, fig.cap = 'Examples of a. Spatially balanced sampling design simulation, b. Preferential sampling design simulation and c. clustered sampling design simulation around the simulated NTZ. Predicted abundance with an 80% increase within the NTZ for d. Yijarup/Pink Snapper (*Chrysophrys auratus*) and e. Southern Maori Wrasse (*Ophthalmolepis lineolatus*)', echo = FALSE, message = FALSE, warning = FALSE}
library(tidyverse); library(sf); library(ggpubr); library(raster); library(terra)

palette <- c("#980000", "#b45f06", "#FFA400", "#41521F", "#073763")
...
ex_spabal <- ggplot() +
    ...
    geom_sf(data = SZ, colour = "#41521F", fill = NA, linewidth = 1) +
    ...
    scale_color_manual(name = '', values = c('Simulated sample points' = 'darkred')) +
    theme_minimal() +
...
ex_pref <- ggplot() +
    ...
    coord_sf(crs = 4326, xlim = ext(sampling_area)[1:2], ylim = ext(sampling_area)[3:4]) +
    scale_color_manual(name = '', values = c('Simulated sample points' = 'darkred', 'Simulated SZ'= "#7f6000")) +
    theme_minimal() +
    theme(axis.text = element_blank(),
          axis.ticks = element_blank())
...
ex_clump <- ggplot() +
    ...
    geom_sf(data = SZ, colour = "#41521F", fill = NA, linewidth = 1) +
    geom_sf(data = aus, color = "darkgray", fill = 'lightgray',  size = 0.2) +
    geom_sf(data = samples_sf_clump, aes(colour = "Simulated sample points")) +
    coord_sf(crs = 4326, xlim = ext(sampling_area)[1:2], ylim = ext(sampling_area)[3:4]) +
    scale_color_manual(name = '', values = c('Simulated sample points' = 'darkred')) +
    theme_minimal() +
    theme(axis.text = element_blank(),
...
PS_raster <- ggplot() +
    geom_raster(data = mature_pred_PS, aes(x = x, y = y, fill = p_mature.fit), interpolate = TRUE) +
...
SMW_raster <- ggplot() +
    geom_raster(data = mature_pred_SMW, aes(x = x, y = y, fill = p_mature.fit), interpolate = TRUE) +
    geom_sf(data = sampling_area, aes(color = 'Simulation area'),
            fill = 'transparent', linewidth = 1) +
    geom_sf(data = SZ, aes(color = 'Simulated NTZ'),
            fill = 'transparent', linewidth = 1) +
...
# Extract common legend from one of the ex_ plots
legend <- get_legend(ex_spabal + theme(legend.position = "right"))

# Arrange the plots
final_plot <- ggarrange(
  ggarrange(ex_spabal + theme(legend.position = "none"), 
            ex_pref + theme(legend.position = "none"), 
            ex_clump + theme(legend.position = "none"), 
            nrow = 1, labels = c("a.", "b.", "c.")),  # First row
  
  ggarrange(PS_raster + theme(legend.position = "none"), 
            SMW_raster + theme(legend.position = "none"), 
            legend,
            nrow = 1, labels = c("d.", "e.", "")),  # Second row
  
  ncol = 1, heights = c(5, 5)  # Adjust heights to balance layout
)
...
Values of abundance were obtained from the latitude and longitude of the `r final_table$Total[final_table$Category == "Simulated BRUVs (preferential sampling design)"]` simulated points of each of the 3000 simulations. To account for the biological variability of each species, and the uncertainty associated with detecting *x* individuals consistently where *x* individuals are predicted to be, each sampling point could observe one abundance value within the Normal distribution of abundance of that raster pixel (rnorm function in base R). We compared the performance of the sampling designs based on several metrics:

-   Control-Impact (CI) comparison: The ratio of the mean abundance of fish inside and outside the simulated NTZ after the abundance increase. This replicates a common monitoring strategy where the effect of the MPA is assessed by comparing the metric of interest in the MPA site (Impact) to that of a similar non-protected site (Control). (Equation 1) [@Underwood1991]
...
## Effect of sampling design on the detection of the NTZ effect of single species
...
best_model_name_PS <- readRDS("data/rmd/gam_best_model_full_data_Chrysophrys_auratus.rds") # variables of best model

# Using the best models above, fitting a GAM to mature fish
best_model_vars_PS <- strsplit(best_model_name_PS, "\\+")[[1]]; best_model_vars_PS
...
best_vars_PS <- make_vars(best_model_vars_PS)
...
m_mature_PS <- make_optimal_model(best_vars_PS, tw())

summary(m_mature_PS)
plot(m_mature_PS, pages = 1, residuals = T, cex = 5)

gam.check(m_mature_PS)
k.check(m_mature_PS)

# Modify the gsub below to match variables in optimal model
tidy_model_PS <- tidy(m_mature_PS)
colnames(tidy_model_PS) <- c("Variable", "Estimate", "Std. Error", "t value", "Pr(>|t|)")
tidy_model_PS$Variable <- gsub("s\\(seagrass_250m\\)", "Seagrass (250m resolution)", tidy_model_PS$Variable)
tidy_model_PS[2:5] <- round(tidy_model_PS[2:5], 3)


# Wrasse ----------------------------------------------------------------------

model_spp <- "Ophthalmolepis lineolatus"
...
best_model_name_SMW <- readRDS("data/rmd/gam_best_model_Ophthalmolepis_lineolatus.rds") # variables of best model, modify the gam below accordingly

# Using the best models above, fitting a GAM to mature fish
best_model_vars_SMW <- strsplit(best_model_name_SMW, "\\+")[[1]]; best_model_vars_SMW
...
best_vars_SMW <- make_vars(best_model_vars_SMW)
...
m_mature_SMW <- make_optimal_model(best_vars_SMW, tw())

summary(m_mature_SMW)

plot(m_mature_SMW, pages = 1, residuals = T, cex = 5)

gam.check(m_mature_SMW)
k.check(m_mature_SMW)

# Modify the gsub below to match variables in optimal model
tidy_model_SMW <- tidy(m_mature_SMW)
colnames(tidy_model_SMW) <- c("Variable", "Estimate", "Std. Error", "t value", "Pr(>|t|)")
tidy_model_SMW$Variable <- gsub("s\\(bathy_250m\\)", "Bathymetry (250m resolution)", tidy_model_SMW$Variable)
tidy_model_SMW$Variable <- gsub("s\\(reef_250m\\)", "Reef (250m resolution)", tidy_model_SMW$Variable)
tidy_model_SMW$Variable <- gsub("s\\(detrended_250m\\)", "Detrended bathymetry (250m resolution)", tidy_model_SMW$Variable)
tidy_model_SMW[2:5] <- round(tidy_model_SMW[2:5], 3)

```

```{r gam_table, tab.cap = "Generalised Additive model of *C.auratus* abundance", echo = FALSE, message = FALSE, warning = FALSE}
knitr::kable(tidy_model_PS)
knitr::kable(tidy_model_SMW)


```

From all the BRUV samples, a distribution model was fitted to bathymetry and habitat covariates. The optimal model for Yijarup/Pink Snapper included seagrass only as a covariate from the 250m resolution dataset. The final model explained `r round(((1 - (deviance(m_mature_PS) / deviance(update(m_mature_PS, . ~ 1))))*100), 2)`% of deviance in the data. The optimal model for (Wadandi name?)/Southern Maori Wrasse included bathymetry, detrended bathymetry and reef, all from the 250m resolution dataset. The final model explained `r round(((1 - (deviance(m_mature_SMW) / deviance(update(m_mature_SMW, . ~ 1))))*100), 2)`% of deviance in the data.

```{r sampling_design_performance_mean_abundance, include = FALSE, echo = FALSE, message = FALSE, warning = FALSE}
SD_points_PS <- readRDS('data/rmd/SD_points_full_data_Chrysophrys_auratus.rds')
SD_points_SMW <- readRDS('data/rmd/SD_points_Ophthalmolepis_lineolatus.rds')

library(tidyverse)
library(kableExtra)

summary_table_PS <- SD_points_PS %>%
  group_by(SD, in_SZ) %>%
...
summary_table_PS[2:5] <- round(summary_table_PS[2:5], 3)

# Wrasse ----------------------------------------------------------------------

summary_table_SMW <- SD_points_SMW %>%
  group_by(SD, in_SZ) %>%
  # Summarize the mean random abundances for before and after
  summarize(
    mean_random_abundance_before = mean(random_abundance_before, na.rm = TRUE),
...
summary_table_SMW[2:5] <- round(summary_table_SMW[2:5], 3)

# Combine ---------------------------------------------------------------------

summary_table_PS <- summary_table_PS %>%
  mutate(Species = "PS")

summary_table_SMW <- summary_table_SMW %>%
  mutate(Species = "SMW")

# Combine the two tables
summary_table_combined <- bind_rows(summary_table_PS, summary_table_SMW) %>%
  relocate(Species, .before = SD)  # Move Species column to the front

# View the combined table
glimpse(summary_table_combined)
...
SD_points_PS <- readRDS('data/rmd/SD_points_Chrysophrys_auratus.rds')
SD_points_SMW <- readRDS('data/rmd/SD_points_Ophthalmolepis_lineolatus.rds')

# Snapper ---------------------------------------------------------------------

dat_PS <- SD_points_PS %>% # Re-organising the dataframe so it's easier to work with.
  pivot_longer(
...
glimpse(percentage_CI_PS) # That is terrible performance my dude.


# Wrasse ----------------------------------------------------------------------

dat_SMW <- SD_points_SMW %>% # Re-organising the dataframe so it's easier to work with.
  pivot_longer(
    cols = c(random_abundance_before, random_abundance_after),
    names_to = "BA",
    values_to = "random_abundance"
...
percentage_CI_SMW <- ratio_CI_SMW %>%
  filter(abundance_ratio >= 1.8*0.9 & abundance_ratio <= 1.8*1.1) %>% # i want only the ratios that are within 10% of the 1.8x increase.
  group_by(SD) %>%  # Group by SD
  summarise(
    perc = n() / nrow(ratio_CI %>% filter(SD == SD)) * 100  # Calculate percentage that's within 10% of 1.8x increase
  )
glimpse(percentage_CI_SMW) # That is terrible performance my dude.

##


files <- list.files(
  path = "outputs/Length_comparison/sampling_designs/ratios/", 
  pattern = "sampling_design_ratio_", 
  full.names = TRUE
)

full_data_Chrysophrys_auratus <- readRDS(files[[4]]) %>% 
  mutate(species = "full data Chrysophrys_auratus") %>% 
  glimpse()

Ophthalmolepis_lineolatus <- readRDS(files[[5]]) %>% 
  mutate(species = "Ophthalmolepis_lineolatus") %>% 
  glimpse()

dat <- rbind(full_data_Chrysophrys_auratus,
             Ophthalmolepis_lineolatus
             )



percentage_after <- dat %>%
...
ggplot(dat, aes(x = abundance_ratio, fill = SD)) +
  geom_histogram(alpha = 0.7, position = "identity", bins = 20) +
  facet_wrap(~ species) +
  scale_fill_manual(values = c("spabal" = "#fff2cc", "pref" = "#f1c232", "clump" = "#7f6000")) +
  labs(title = paste("Abundance Ratio Distributions inside/outside \n", spp), x = "abundance ratio inside/outside", y = "Count") +
...
ggplot(dat, aes(x = SD, y = abundance_ratio, fill = SD)) +
  annotate("rect", xmin = -Inf, xmax = Inf, ymin = 1.7, ymax = 1.9, 
           fill = "red", alpha = 0.2) + 
  geom_hline(yintercept = 1.8, color = "red", linetype = "dashed", linewidth = 1) +
...
count_data <- dat %>%
  group_by(species, SD) %>%
  summarize(
    count_in_range = sum(abundance_ratio >= 1.7 & abundance_ratio <= 1.9),
    total_count = n(),  # Total number of rows for that species and SD
    .groups = "drop"
  ) %>%
  mutate(percentage = (count_in_range / total_count) * 100)  # Calculate percentage
...
ggplot(dat, aes(x = SD, y = abundance_ratio, fill = SD)) +
  ...
  labs(x = "SD",
       y = "Abundance Ratio") +
  theme_minimal() +
  geom_text(data = count_data, aes(x = SD, y = 1.95, label = paste0(round(percentage, 1), "%")), 
            color = "black", fontface = "bold", size = 5)  # Display % instead of count


ggplot(dat, aes(x = abundance_ratio, fill = SD)) +
  #annotate("rect", xmin = -Inf, xmax = Inf, ymin = 1.7, ymax = 1.9, 
  #         fill = "red", alpha = 0.2) + 
  #geom_vline(yintercept = 1.8, color = "red", linetype = "dashed", linewidth = 1) +
  geom_density(alpha = 0.7) +  # Density plot for abundance_ratio
  facet_wrap(~ species) +  # Separate by species
  scale_fill_manual(values = c("spabal" = "#fff2cc", "pref" = "#f1c232", "clump" = "#7f6000")) +
  labs(x = "Abundance Ratio",
       y = "Density") +
  theme_minimal() +
  geom_text(data = count_data, aes(x = 2, y = 1.95, label = paste0(round(percentage, 1), "%")), 
            color = "black", fontface = "bold", size = 5)  # Display % above the shaded region
...
In this study, the performance of preferential and spatially balanced BRUV sampling designs are compared as MPA monitoring tools for single species detection and community composition at the local scale. Results highlight the importance of sampling design choice for BRUV studies, and the estimation tendency of different spatial sampling strategies on fish abundance
